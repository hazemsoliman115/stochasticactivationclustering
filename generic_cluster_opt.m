function [x_new, u_new, q_new] = generic_cluster_opt(No_APs, No_Areas, No_time_slots, epsilon, p_mat, sigma_sq, A_e)
...
while sum(sum(abs(x_new-x_old)))/sum(sum(abs(x_old))) > epsilon
    ...
    for t=1:No_time_slots
        ...
        for i=1:No_Areas
            ...
            for j =1:No_APs
                for k=1:No_APs
                    if k ~= j
                        den(i) = den(i) + u_old(t,i,j)*p_mat(i,k)*x_old(t,k);
                    end
                end
            end
            den(i) = den(i) + sum(reshape(u_old(t,i,:),size(x_old(t,:))).*p_mat(i,:).*x_old(t,:));
            for j =1:No_APs
                alpha_matrix(t,i,j) = ( u_old(t,i,j)*p_mat(i,j)*x_old(t,j) )/( den(i) );
                for k=1:No_APs
                    if k ~= j
                        alpha_cluster(t,i,j,k) = ( gam(i)*Q_e_old(t,i)*u_old(t,i,j)*q_old(t,j,k)*p_mat(i,k)*x_old(t,k) )/( den(i) );
                    end
                end
            end
        end
    ...
    for t=2:No_time_slots
        ...
        for i=1:No_Areas
            ...
            for j =1:No_APs
                for k=1:No_APs
                    if k ~= j
                        den_Queue_den(i) = den_Queue_den(i) + u_old(t-1,i,j)*p_mat(i,k)*x_old(t-1,k);
                    end
                end
            end
            for j =1:No_APs
                for k=1:No_APs
                    if k ~= j
                        alpha_Q_den(t-1,i,j,k) = ( u_old(t-1,i,j)*p_mat(i,k)*x_old(t-1,k) )/( den_Queue_den(i) );
                    end
                end
            end
        end
        ...
        for i=1:No_Areas
            ...
            for j =1:No_APs
                for k=1:No_APs
                    if k ~= j
                        den_Queue_nom(i) = den_Queue_nom(i) + u_old(t-1,i,j)*q_old(t-1,j,k)*p_mat(i,k)*x_old(t-1,k);
                    end
                end
            end
            den_Queue_nom(i) = den_Queue_nom(i) + Q_e_old(t,i)*(Q_e_old(t-1,i))^(-1)/(A_e(t-1,i))*sum(reshape(u_old(t-1,i,:),size(x_old(t-1,:))).*p_mat(i,:).*x_old(t-1,:));
            for j =1:No_APs
                alpha_Q_nom_Pow(t-1,i,j) = ( Q_e_old(t,i)*(Q_e_old(t-1,i))^(-1)/(A_e(t-1,i))*u_old(t-1,i,j)*p_mat(i,j)*x_old(t-1,j) )/( den_Queue_nom(i) );
                for k=1:No_APs
                    if k ~= j
                        alpha_Q_nom_Intf(t-1,i,j,k) = ( u_old(t-1,i,j)*q_old(t-1,j,k)*p_mat(i,k)*x_old(t-1,k) )/( den_Queue_nom(i) );
                    end
                end
            end
        end
    ...
    %%
    %options = sdpsettings('solver','mosek-geometric','mosek.MSK_IPAR_INFEAS_REPORT_AUTO','MSK_ON','mosek.MSK_IPAR_INFEAS_REPORT_LEVEL',1);
    ...
    for t=1:No_time_slots
        for i=1:No_Areas
            ...
            for j=1:No_APs
                for k=1:No_APs
                    if k ~= j
                        z(t,i) = z(t,i) + u(t,i,j)*p_mat(i,k)*x(t,k);
                        nom(t,i) = nom(t,i)*(u(t,i,j)*q(t,j,k)*p_mat(i,k)*x(t,k)/alpha_cluster(t,i,j,k))^(-alpha_cluster(t,i,j,k));
                    end
                end
            end
            ...
            Constr_1(t,i) = (gam(i)*Q_e(t,i))*z(t,i)*nom(t,i)*prod( (reshape(u(t,i,:),size(x(t,:))).*p_mat(i,:).*x(t,:)./reshape(alpha_matrix(t,i,:),size(x(t,:)))).^(-reshape(alpha_matrix(t,i,:),size(x(t,:)))) );
        end
    end
    toc
    ...
    for t=2:No_time_slots
        for i=1:No_Areas
            ...
            for j=1:No_APs
                for k=1:No_APs
                    if k ~= j
                        z_Q_den(t-1,i) = z_Q_den(t-1,i) + u(t-1,i,j)*q(t-1,j,k)*p_mat(i,k)*x(t-1,k);
                        nom_Q_den(t-1,i) = nom_Q_den(t-1,i)*(u(t-1,i,j)*p_mat(i,k)*x(t-1,k)/alpha_Q_den(t-1,i,j,k))^(-alpha_Q_den(t-1,i,j,k));
                    end
                end
            end
            ...
            Constr_2(t-1,i) = ( Q_e(t,i)*(Q_e(t-1,i))^(-1)/(A_e(t-1,i))*sum(reshape(u(t-1,i,:),size(x(t-1,:))).*p_mat(i,:).*x(t-1,:)) + z_Q_den(t-1,i) )*nom_Q_den(t-1,i);
        end
    end
    toc
    ...
    for t=2:No_time_slots
        for i=1:No_Areas
            ...
            for j=1:No_APs
                for k=1:No_APs
                    if k ~= j
                        z_Q_nom(t-1,i) = z_Q_nom(t-1,i) + u(t-1,i,j)*p_mat(i,k)*x(t-1,k);
                        nom_Q_nom(t-1,i) = nom_Q_nom(t-1,i)*(u(t-1,i,j)*q(t-1,j,k)*p_mat(i,k)*x(t-1,k)/alpha_Q_nom_Intf(t-1,i,j,k))^(-alpha_Q_nom_Intf(t-1,i,j,k));
                    end
                end
            end
            ...
            Constr_3(t-1,i) = s(t,i)^(-1)*z_Q_nom(t-1,i)*nom_Q_nom(t-1,i)*prod( (Q_e(t,i)*(Q_e(t-1,i))^(-1)/(A_e(t-1,i))*reshape(u(t-1,i,:),size(x(t-1,:))).*p_mat(i,:).*x(t-1,:)./reshape(alpha_Q_nom_Pow(t-1,i,:),size(x(t-1,:)))).^(-reshape(alpha_Q_nom_Pow(t-1,i,:),size(x(t-1,:)))) )*nom_Q_nom(t-1,i);
        end
    end
    toc
...
%     Constr = [ Constr , 0.1 <= Q_e];
    ...
    Constr = [ 0 <= Constr_1 <= 1, 0 <= Constr_2 <= 1, 0 <= Constr_3 <= 1, 0.1 <= x <= 1, 1 <= s <= 1.1, 0.1 <= u <= 1, 0.1 <= q <= 1, 0.1 <= Q_e];
    toc
     for t=1:No_time_slots
         for i=1:No_Areas
